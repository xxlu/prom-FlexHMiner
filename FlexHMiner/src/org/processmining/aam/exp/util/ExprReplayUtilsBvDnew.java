...
import nl.tue.alignment.ReplayerParameters.AStar;
 ...
 * 
 */
public class ExprReplayUtilsBvDnew {
...
//				newClasses, etMapping, 1, 1);
		...
		boolean usePartialOrder = true;
		
		int costUpperBound = Integer.MAX_VALUE;
		// timeout per trace in milliseconds
		int timeoutMilliseconds = 10 * 1000;
		
		AStar replayParam = new ReplayerParameters.AStar(true, true, true, numThreads, false,
				Debug.NONE, timeoutMilliseconds, maximumNumberOfStates,Integer.MAX_VALUE, usePartialOrder);
...
//				replayResult = replayer.computePNRepResult(context.getProgress(), log);
				 ...
				 * -
				 */
				quality.fitness = ExprReplayUtilsBvDnew.getAlignmentValue(replayResult);
...
//		}
	...
	public static void computeGeneralizationMeasures(PluginContext context, XLog newLowLog, ExpDiscoveryAlgorithm alg, ModelQuality quality, boolean useILP) {
		...
		for(int i = 0 ; i < trainLogs.size() ; i++ ) {
			...
			fitnesslist[i] = ExprReplayUtilsBvDnew.computeMeasures(context, testLog, markedNet.getPetrinet(),
					markedNet.getInitialMarking(), markedNet.getFinalMarking()).fitness;
			
			// Compute precision using full log
			precisionlist[i] = ExprReplayUtilsBvDnew.computeMeasures(context, newLowLog, markedNet.getPetrinet(),
